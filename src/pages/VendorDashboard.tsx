...
export const VendorDashboard: React.FC = () => {
  const {
    ...
    products,
    reviews,
    enquiries,          // ← from AppContext state (Supabase-backed)
    promotions,
    adminSettings,
    isLoading,
  } = useApp();
  ...
  // ── Enquiries local state ──────────────────────────────────────────────────
  // We derive from AppContext enquiries (Supabase) rather than StorageManager.getEnquiries()
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-3xl max-w-md w-full text-center space-y-4 border border-slate-200">
          ...
          <button
            ...
            className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-6 py-2.5 rounded-xl text-xs transition shadow-sm cursor-pointer"
          >
  ...
  const myProducts = useMemo(
    ...
    [products, activeVendor.id]
  );

  const myReviews = useMemo(
    () => reviews.filter((r) => r.vendorId === activeVendor.id),
    [reviews, activeVendor.id]
  );

  // Derive enquiries from AppContext state (not localStorage)
  const myEnquiries = useMemo(
    () =>
      enquiries
        .filter((e) => e.vendorId === activeVendor.id)
        .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()),
    [enquiries, activeVendor.id]
  );

  const unreadEnquiries = useMemo(
    () => myEnquiries.filter((e) => !e.read).length,
    [myEnquiries]
  );
  ...
  // ── Save profile — writes to Supabase, realtime UPDATE patches AppContext state ──
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    try {
      // updateVendorAsync writes to Supabase; realtime UPDATE event in AppContext
      // patches the vendors state automatically — no manual refreshData() needed.
      await StorageManager.updateVendorAsync(updatedVendor);
      showToast('success', 'Store Profile Updated!', 'Your business profile has been saved.');
    } catch (err: any) {
      showToast('error', 'Save Failed', err?.message || 'Could not save profile. Please try again.');
    }
  };
  ...
  const handleVerifyNIN = async () => {
    ...
    if (res.success && res.data) {
      ...
      try {
        await StorageManager.updateVendorAsync(updated);
        showToast('success', 'NIMC Identity Verified!', 'NIN match confirmed with NIMC government database.');
      } catch (err: any) {
        showToast('error', 'Save Failed', err?.message || 'NIN verified but could not save. Please retry.');
      }
    } else {
  ...
  const handleDeleteProduct = async (prodId: string) => {
    ...
    await StorageManager.deleteProductAsync(prodId);
    // Products in AppContext update via StorageManager's realtime channel
    showToast('info', 'Product Deleted', 'Item removed from your store catalogue.');
  };

  // ── Enquiry actions — write to Supabase, then call refreshData() ───────────
  // refreshData() is cheap for enquiries because AppContext re-reads from
  // StorageManager.getEnquiries() (localStorage mirror of Supabase enquiries).
  // We do NOT call refreshData() for vendor-unrelated reasons.
  const handleMarkEnquiryRead = async (enqId: string) => {
    await StorageManager.markEnquiryReadAsync(enqId);
    await refreshData();
    showToast('success', 'Marked as Read', 'Enquiry status updated.');
  };

  const handleSendDirectReply = async (enq: Enquiry) => {
    const text = replyTextMap[enq.id]?.trim();
    if (!text) {
      showToast('error', 'Empty Reply', 'Please enter a reply message before submitting.');
      return;
    }
    await StorageManager.replyEnquiryAsync(enq.id, text);
    await refreshData();
  ...
  const handleWhatsAppReply = async (enq: Enquiry) => {
    const text =
  ...
  const handleDeleteEnquiry = async (enqId: string) => {
    if (!confirm('Are you sure you want to delete this enquiry from your dashboard?')) return;
    await StorageManager.deleteEnquiryAsync(enqId);
    await refreshData();
    ...
    <div className="min-h-screen bg-slate-100 flex flex-col lg:flex-row">
      ...
      {mobileDrawerOpen && (
        <>
          ...
          />
          <div className="fixed inset-y-0 left-0 w-72 max-w-[82vw] bg-slate-900 text-white z-50 p-4 sm:p-5 flex flex-col justify-between shadow-2xl lg:hidden overflow-y-auto">
            <div className="space-y-5">
              <div className="pb-3 border-b border-slate-800 flex items-center justify-between">
                ...
                <button onClick={() => setMobileDrawerOpen(false)} className="p-1.5 text-slate-400 hover:text-white rounded-lg bg-slate-800">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <nav className="space-y-1 text-xs font-semibold text-slate-300">
                {[
                  ...
                  { id: 'enquiries', label: 'WhatsApp Enquiries', icon: <MessageSquare className="w-4 h-4" />, badge: unreadEnquiries > 0 ? `${unreadEnquiries} NEW` : null },
                  { id: 'reviews', label: 'Reviews', icon: <Star className="w-4 h-4 text-amber-400" />, suffix: `${activeVendor.rating}★` },
                  { id: 'nin', label: 'NIN Verification', icon: <ShieldCheck className="w-4 h-4 text-amber-400" />, special: true },
                  { id: 'settings', label: 'Edit Profile', icon: <Settings className="w-4 h-4" /> },
                  { id: 'analytics', label: 'Analytics', icon: <TrendingUp className="w-4 h-4" /> },
                  { id: 'promotions', label: 'Promotions', icon: <Sparkles className="w-4 h-4 text-amber-400" />, badge: myActivePromotions.length > 0 ? `${myActivePromotions.length} Active` : null },
                ].map(({ id, label, icon, badge, suffix, special }) => (
                  <button
                    key={id}
                    onClick={() => { setActiveTab(id as any); setMobileDrawerOpen(false); }}
                    className={`w-full text-left px-3 py-2.5 rounded-xl flex items-center justify-between transition ${
                      activeTab === id
                        ? special ? 'bg-amber-600 text-white font-bold' : 'bg-orange-600 text-white font-bold'
                        : special && !activeVendor.ninVerified
                        ? 'bg-amber-950/60 text-amber-300 border border-amber-800/50'
                        : 'hover:bg-slate-800'
                    }`}
                  >
                    <span className="flex items-center gap-2.5">{icon} {label}</span>
                    {badge && <span className={`text-[10px] font-black px-1.5 py-0.5 rounded-full ${id === 'enquiries' ? 'bg-rose-500 text-white' : 'bg-emerald-500 text-white'}`}>{badge}</span>}
                    {suffix && <span className="text-amber-400 font-bold text-[11px]">{suffix}</span>}
                    {special && (activeVendor.ninVerified
                      ? <span className="text-green-400 text-[10px] font-bold">✓ Verified</span>
                      : <span className="bg-amber-400 text-slate-950 text-[9px] font-black px-1.5 py-0.5 rounded uppercase">REQUIRED</span>
                    )}
                  </button>
                ))}
                <button
      ...
      <div className="lg:hidden bg-white border-b border-slate-200 px-3 py-2 overflow-x-auto flex items-center gap-2 text-xs font-semibold shrink-0 sticky top-[53px] z-20 shadow-2xs no-scrollbar">
        {[
          { id: 'dashboard', label: 'Overview', icon: <LayoutDashboard className="w-4 h-4" /> },
          { id: 'products', label: `Products (${myProducts.length})`, icon: <Package className="w-4 h-4" /> },
          { id: 'enquiries', label: 'Enquiries', icon: <MessageSquare className="w-4 h-4" />, badge: unreadEnquiries },
          { id: 'reviews', label: `Reviews (${activeVendor.rating}★)`, icon: <Star className="w-4 h-4 text-amber-500" /> },
          { id: 'nin', label: `NIN ${activeVendor.ninVerified ? '✓' : 'REQUIRED'}`, icon: <ShieldCheck className="w-4 h-4 text-amber-600" />, amber: true },
          { id: 'promotions', label: 'Ads & Promos', icon: <Sparkles className="w-4 h-4 text-amber-500" /> },
          { id: 'settings', label: 'Settings', icon: <Settings className="w-4 h-4" /> },
        ].map(({ id, label, icon, badge, amber }) => (
          <button
            key={id}
            ...
            className={`px-3.5 py-2.5 min-h-[44px] rounded-full whitespace-nowrap transition flex items-center gap-1.5 shrink-0 active:scale-95 ${
              activeTab === id
                ? amber ? 'bg-amber-600 text-white font-bold shadow-xs' : 'bg-orange-600 text-white font-bold shadow-xs'
                : amber && !activeVendor.ninVerified
                ? 'bg-amber-100 text-amber-900 border border-amber-300'
                : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
            }`}
          >
            {icon} {label}
            {badge ? <span className="bg-rose-500 text-white text-[9px] px-1.5 py-0.5 rounded-full font-black">{badge}</span> : null}
          </button>
        ))}
      </div>
      ...
      <aside className="hidden lg:flex lg:flex-col lg:w-64 bg-slate-900 text-white p-5 space-y-6 shrink-0 border-r border-slate-800 min-h-screen">
        ...
        <nav className="space-y-1.5 text-xs font-semibold text-slate-300">
          {[
            ...
            { id: 'products', label: 'Catalogue Products', icon: <Package className="w-4 h-4" />, count: myProducts.length },
            { id: 'enquiries', label: 'WhatsApp Enquiries', icon: <MessageSquare className="w-4 h-4" />, badge: unreadEnquiries > 0 ? unreadEnquiries : null },
            { id: 'reviews', label: 'Customer Reviews', icon: <Star className="w-4 h-4 text-amber-400" />, suffix: `${activeVendor.rating}★` },
            { id: 'nin', label: 'NIN Verification', icon: <ShieldCheck className="w-4 h-4 text-amber-400" />, special: true },
            { id: 'settings', label: 'Edit Store Profile', icon: <Settings className="w-4 h-4" /> },
            { id: 'analytics', label: 'Store Analytics', icon: <TrendingUp className="w-4 h-4" /> },
            { id: 'promotions', label: 'Promotions & Add-ons', icon: <Sparkles className="w-4 h-4 text-amber-400" />, activeBadge: myActivePromotions.length > 0 ? myActivePromotions.length : null },
          ].map(({ id, label, icon, count, badge, suffix, special, activeBadge }) => (
            <button
              key={id}
              onClick={() => setActiveTab(id as any)}
            ...
            >
              <span className="flex items-center gap-2.5">{icon} {label}</span>
              {count !== undefined && <span className="bg-slate-800 text-slate-300 px-2 py-0.5 rounded-full text-[10px]">{count}</span>}
              {badge && <span className="bg-rose-500 text-white px-2 py-0.5 rounded-full text-[10px] font-bold">{badge}</span>}
              {suffix && <span className="text-amber-400 font-bold">{suffix}</span>}
              {activeBadge && <span className="bg-emerald-500 text-white px-2 py-0.5 rounded-full text-[10px] font-bold">{activeBadge} Active</span>}
              {special && (
      ...
      <main className="flex-1 p-3.5 sm:p-6 lg:p-8 space-y-4 sm:space-y-6 max-w-6xl mx-auto pb-28 lg:pb-12 min-w-0">
          ...
          <div className="flex flex-col xs:flex-row items-start xs:items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {activeVendor.isLive ? (
                    <span className="bg-emerald-50 text-emerald-700 border border-emerald-200 text-[10px] font-extrabold px-2 py-0.5 rounded-full flex items-center gap-1 shrink-0">
                      <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span> Live Store
                    </span>
                  ) : (
            ...
            <div className="flex items-center gap-2 w-full xs:w-auto shrink-0 pt-1 xs:pt-0 border-t xs:border-0 border-slate-100">
              <button
                ...
                className="flex-1 xs:flex-initial bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold px-3.5 py-2.5 min-h-[44px] rounded-xl text-xs flex items-center justify-center gap-1.5 transition"
              >
                <Edit className="w-4 h-4 text-slate-600" /> Edit Profile
              </button>
              <button
                onClick={() => navigateToStore(activeVendor.slug)}
                className="flex-1 xs:flex-initial bg-orange-600 hover:bg-orange-700 text-white font-bold px-4 py-2.5 min-h-[44px] rounded-xl text-xs flex items-center justify-center gap-1.5 shadow-xs transition"
              >
        ...
        {!activeVendor.isLive ? (
          <div className="bg-gradient-to-r from-amber-500 to-amber-600 text-slate-950 p-4 sm:p-6 rounded-2xl sm:rounded-3xl shadow-xs space-y-3 sm:space-y-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs font-bold pt-1">
              {[
                ...
                { label: 'WhatsApp Verified', done: true },
                { label: 'NIN Verification', done: activeVendor.ninVerified },
                { label: 'Admin Review', done: false, clock: true },
              ].map(({ label, done, clock }) => (
                <div key={label} className="flex items-center gap-1.5">
                  {done ? <CheckCircle2 className="w-4 h-4 text-slate-950 shrink-0" /> : clock ? <Clock className="w-4 h-4 text-slate-950 shrink-0" /> : <AlertCircle className="w-4 h-4 text-rose-900 shrink-0" />}
          ...
          <div className="bg-gradient-to-r from-emerald-800 to-emerald-950 text-white p-4 sm:p-6 rounded-2xl sm:rounded-3xl shadow-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto shrink-0">
              <button onClick={() => setActiveTab('settings')} className="flex-1 sm:flex-initial bg-emerald-900 hover:bg-emerald-800 text-white font-bold px-3.5 py-2.5 min-h-[44px] rounded-xl text-xs flex items-center justify-center gap-1.5 border border-emerald-700/60 shadow-xs">
                <Settings className="w-4 h-4 text-emerald-300" /> Edit Profile
              </button>
              <button onClick={() => navigateToStore(activeVendor.slug)} className="flex-1 sm:flex-initial bg-white text-emerald-950 hover:bg-emerald-50 font-bold px-4 py-2.5 min-h-[44px] rounded-xl text-xs flex items-center justify-center gap-1.5 shadow-xs">
                <Store className="w-4 h-4 text-emerald-600" /> Open Store
              </button>
            </div>
          </div>
        )}
          ...
          <div className="space-y-4 sm:space-y-6">
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5 sm:gap-5">
                <div className="bg-white p-4 sm:p-5 rounded-2xl sm:rounded-3xl border border-slate-200 shadow-xs hover:shadow-md transition space-y-3 flex flex-col justify-between">
                  ...
                  <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-500">
                    <span>Ikorodu Visibility</span><span className="font-bold text-blue-600">High Traffic</span>
                  </div>
                </div>
                ...
                <div className="bg-white p-4 sm:p-5 rounded-2xl sm:rounded-3xl border border-slate-200 shadow-xs hover:shadow-md transition space-y-3 flex flex-col justify-between">
                  ...
                  <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-500">
                    <span>Direct WhatsApp & Chat</span><span className="font-bold text-emerald-600">100% Direct</span>
                  </div>
                </div>

                <div className="bg-white p-4 sm:p-5 rounded-2xl sm:rounded-3xl border border-slate-200 shadow-xs hover:shadow-md transition space-y-3 sm:col-span-2 lg:col-span-1 flex flex-col justify-between">
                  <div className="flex items-center justify-between">
                  ...
                  <div>
                    <div className="flex items-baseline gap-2">
                      ...
                      <span className="text-[10px] sm:text-xs font-bold text-purple-700 bg-purple-50 px-2 py-0.5 rounded-full flex items-center gap-1">
                        <span className="w-1.5 h-1.5 rounded-full bg-purple-600 animate-pulse"></span> Live Tracker
                      </span>
                    </div>
                  ...
                  <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-500">
                    <span>Interaction History</span><span className="font-bold text-purple-600">Updated Real-Time</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
              {[
                { label: 'Profile Views', value: activeVendor.analytics.profileViews.toLocaleString(), color: 'text-slate-900' },
                { label: 'WhatsApp Taps', value: activeVendor.analytics.whatsappTaps.toLocaleString(), color: 'text-emerald-600' },
                { label: 'Catalogue Products', value: myProducts.length, color: 'text-slate-900' },
                { label: 'Customer Rating', value: `${activeVendor.rating} ★`, color: 'text-amber-500' },
              ].map(({ label, value, color }) => (
                <div key={label} className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-xs space-y-1">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {unreadEnquiries > 0 && (
                      <span className="bg-rose-500 text-white text-[10px] font-black px-2 py-0.5 rounded-full animate-pulse">{unreadEnquiries} NEW</span>
                    )}
                  </div>
                ...
                >
                  <MessageSquare className="w-3.5 h-3.5 text-orange-400" /> View All ({myEnquiries.length})
                </button>
              </div>
              {myEnquiries.length === 0 ? (
                  ...
                  {myEnquiries.slice(0, 3).map((enq) => (
                    <div key={enq.id} className={`p-3.5 rounded-2xl border text-xs space-y-2 transition ${!enq.read ? 'bg-amber-50/60 border-amber-300' : 'bg-slate-50 border-slate-200'}`}>
                      ...
                      <div className="flex items-center justify-between pt-1 text-[11px]">
                        ...
                        <button
                          onClick={() => { setActiveTab('enquiries'); setReplyingEnquiryId(enq.id); }}
            ...
            <div className="bg-white rounded-3xl p-6 border border-slate-200 space-y-4 shadow-xs">
              ...
              {myProducts.length === 0 ? (
                <div className="text-center py-8 bg-slate-50 rounded-2xl border border-dashed border-slate-300 space-y-2">
                  ...
                  <button onClick={() => { setProductToEdit(null); setProductModalOpen(true); }} className="bg-emerald-600 text-white font-bold text-xs px-4 py-2 rounded-xl">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-col sm:flex-row gap-2">
                    <input
                      type="text" maxLength={11} placeholder="e.g. 12345678901"
                      value={ninInput} onChange={(e) => setNinInput(e.target.value.replace(/\D/g, ''))}
                    ...
                    {ninInput.length === 11 && (
                      <button
                        onClick={handleVerifyNIN} disabled={ninLoading}
                        className="bg-amber-600 hover:bg-amber-700 disabled:opacity-50 text-white font-bold text-xs px-5 py-3 rounded-xl shadow-xs transition flex items-center justify-center gap-1.5 shrink-0 min-h-[44px]"
                      >
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-slate-100">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <div className="relative">
                  ...
                  <input
                    type="text" placeholder="Search name, phone, message..."
                    value={enquirySearch} onChange={(e) => setEnquirySearch(e.target.value)}
                ...
                <div className="flex bg-slate-100 p-1 rounded-xl text-xs font-bold text-slate-600 border border-slate-200">
                  {(['all', 'unread', 'replied'] as const).map((f) => (
                    <button
                      key={f} onClick={() => setEnquiryFilter(f)}
                      className={`px-3 py-1.5 rounded-lg transition ${enquiryFilter === f ? 'bg-white text-slate-900 shadow-xs' : 'hover:text-slate-900'}`}
                    >
                      {f === 'all' ? `All (${myEnquiries.length})` : f === 'unread' ? `Unread (${unreadEnquiries})` : `Replied (${myEnquiries.filter((e) => e.replyText).length})`}
                    </button>
                  ))}
                </div>
              </div>
            ...
            {filteredEnquiries.length === 0 ? (
              <div className="text-center py-12 bg-slate-50 rounded-2xl border border-slate-200 space-y-3">
                ...
                <p className="text-xs text-slate-500 max-w-sm mx-auto">
                  {enquirySearch ? 'No messages found matching your search. Try clearing the search box.' : 'When buyers tap "Chat with Vendor" on your store, messages appear here instantly.'}
                </p>
                  ...
                  <div key={enq.id} className={`p-5 rounded-2xl border transition space-y-3 ${!enq.read ? 'bg-amber-50/50 border-amber-300 shadow-xs' : 'bg-white border-slate-200 hover:border-slate-300'}`}>
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-2 border-b border-slate-100">
                        ...
                        <div>
                          ...
                          {enq.productName && (
                            <p className="text-xs font-semibold text-orange-700 mt-0.5 flex items-center gap-1">
                              <Package className="w-3.5 h-3.5 text-orange-500" /> Product Inquiry: <strong>{enq.productName}</strong>
                            </p>
                          )}
                        </div>
                      </div>
                      <div className="flex items-center gap-2 shrink-0 self-end sm:self-auto">
                        <span className="text-[11px] text-slate-400 font-medium">
                          {new Date(enq.createdAt).toLocaleString([], { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                        </span>
                        {!enq.read ? (
                          <button onClick={() => handleMarkEnquiryRead(enq.id)} className="bg-amber-200 hover:bg-amber-300 text-amber-900 text-[10px] font-black px-2 py-1 rounded-lg transition">● New (Mark Read)</button>
                        ) : (
                          <span className="text-[10px] text-slate-400 bg-slate-100 px-2 py-0.5 rounded-md font-bold">Read</span>
                        )}
                      </div>
                    </div>
                    ...
                    {replyingEnquiryId === enq.id ? (
                      <div className="bg-slate-900 text-white p-4 rounded-2xl space-y-3 border border-slate-800">
                        ...
                        <textarea
                          rows={3} value={replyTextMap[enq.id] || ''}
                          onChange={(e) => setReplyTextMap({ ...replyTextMap, [enq.id]: e.target.value })}
                          placeholder={`Hi ${enq.customerName}, thank you for your enquiry!...`}
                        ...
                        <div className="flex items-center gap-2 pt-1">
                          <button onClick={() => handleSendDirectReply(enq)} className="flex-1 bg-orange-600 hover:bg-orange-700 text-white font-extrabold text-xs py-2.5 rounded-xl transition flex items-center justify-center gap-1.5 shadow-sm">
                            <Send className="w-3.5 h-3.5" /> Save Dashboard Reply
                          </button>
                          <button onClick={() => handleWhatsAppReply(enq)} className="bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-xs px-4 py-2.5 rounded-xl transition flex items-center gap-1.5 shadow-sm">
                      ...
                      <div className="flex items-center justify-between pt-1 flex-wrap gap-2 text-xs">
                          ...
                          <button
                            onClick={() => {
                              ...
                              if (!replyTextMap[enq.id]) {
                                setReplyTextMap({ ...replyTextMap, [enq.id]: `Hello ${enq.customerName}, thank you for reaching out to ${activeVendor.businessName}! ` });
                              }
                            }}
                            className="bg-slate-900 hover:bg-slate-800 text-white font-bold px-3.5 py-2.5 min-h-[44px] rounded-xl flex items-center justify-center gap-1.5 transition text-xs shadow-2xs flex-1 sm:flex-initial"
                          >
                            <Reply className="w-4 h-4 text-orange-400" />
                            <span>{enq.replyText ? 'Edit Reply' : 'Reply Message'}</span>
                          </button>
                          <button onClick={() => handleWhatsAppReply(enq)} className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-3.5 py-2.5 min-h-[44px] rounded-xl flex items-center justify-center gap-1.5 transition text-xs shadow-2xs flex-1 sm:flex-initial">
                            <MessageCircle className="w-4 h-4" /> Reply via WhatsApp
                          </button>
                          <a href={`tel:${enq.customerPhone}`} className="bg-slate-100 hover:bg-slate-200 text-slate-800 font-semibold px-3.5 py-2.5 min-h-[44px] rounded-xl flex items-center justify-center gap-1.5 transition text-xs border border-slate-300">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button onClick={() => handleDeleteEnquiry(enq.id)} className="text-rose-500 hover:text-rose-700 font-semibold text-xs py-1 px-2 flex items-center gap-1" title="Delete Enquiry">
        ...
        {/* ════════════════════════════════════════════════════════════════════
            TAB: SETTINGS (Edit Store Profile)
        ════════════════════════════════════════════════════════════════════ */}
        {activeTab === 'settings' && (
          <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 space-y-8 shadow-xs max-w-4xl mx-auto">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100">
              ...
              <button
                type="button" onClick={() => navigateToStore(activeVendor.slug)}
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-end gap-3 -mt-10 sm:-mt-12">
                    ...
                    <div className="space-y-1 pt-1 sm:pt-0">
                      ...
                      <p className="text-xs text-slate-600 font-medium flex items-center gap-1.5">
                        <MapPin className="w-3.5 h-3.5 text-emerald-600 shrink-0" /> {formAddress || 'Physical Address in Ikorodu'}
                      </p>
            ...
            <form onSubmit={handleSaveProfile} className="space-y-8">
              ...
              <div className="space-y-6 pt-4 border-t border-slate-100">
                <div className="flex items-center gap-2 text-slate-900 font-extrabold text-base">
                  <Camera className="w-5 h-5 text-orange-600" /> Store Branding & Photos
                </div>
                  ...
                  <div className="bg-slate-50 p-5 rounded-2xl border border-slate-200 space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="w-12 h-12 rounded-xl bg-white p-1 border border-slate-200 overflow-hidden shrink-0 shadow-xs">
                        <img src={formLogoURL || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=500&auto=format&fit=crop'} alt="Logo" className="w-full h-full object-cover rounded-lg" />
                      </div>
                    </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <input type="url" placeholder="https://..." value={formLogoURL} onChange={(e) => setFormLogoURL(e.target.value)} className="w-full px-3 py-2 rounded-xl border border-slate-300 text-xs focus:ring-2 focus:ring-orange-500 outline-none bg-white text-slate-900" />
                    </div>
                    <div className="space-y-1.5 pt-1">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {PRESET_LOGOS.map((preset, idx) => (
                          <button key={idx} type="button" onClick={() => setFormLogoURL(preset.url)} className="text-[10px] bg-white hover:bg-orange-50 hover:text-orange-600 text-slate-700 font-semibold px-2.5 py-1 rounded-lg border border-slate-200 transition">{preset.label}</button>
                        ))}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-50 p-5 rounded-2xl border border-slate-200 space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="w-16 h-10 rounded-xl bg-white border border-slate-200 overflow-hidden shrink-0 shadow-xs">
                        <img src={formCoverURL || 'https://images.unsplash.com/photo-1441986300917-64674bd600d8?w=1200&auto=format&fit=crop'} alt="Cover" className="w-full h-full object-cover" />
                      </div>
                    </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <input type="url" placeholder="https://..." value={formCoverURL} onChange={(e) => setFormCoverURL(e.target.value)} className="w-full px-3 py-2 rounded-xl border border-slate-300 text-xs focus:ring-2 focus:ring-orange-500 outline-none bg-white text-slate-900" />
                    </div>
                    <div className="space-y-1.5 pt-1">
                      <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Quick Sample Banners:</p>
                      <div className="flex flex-wrap gap-1.5">
                        {PRESET_COVERS.map((preset, idx) => (
                          <button key={idx} type="button" onClick={() => setFormCoverURL(preset.url)} className="text-[10px] bg-white hover:bg-orange-50 hover:text-orange-600 text-slate-700 font-semibold px-2.5 py-1 rounded-lg border border-slate-200 transition">{preset.label}</button>
                        ))}
                      </div>
                    </div>
                  </div>
              ...
              <div className="space-y-4 pt-6 border-t border-slate-100">
                <div className="flex items-center gap-2 text-slate-900 font-extrabold text-base">
                  <Building className="w-5 h-5 text-orange-600" /> Business Information & Category
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-700">Business Name *</label>
                    <input type="text" required value={formBusinessName} onChange={(e) => setFormBusinessName(e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-orange-500 outline-none font-semibold text-slate-900" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-700">Owner / Representative Name *</label>
                    <input type="text" required value={formOwnerName} onChange={(e) => setFormOwnerName(e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-orange-500 outline-none text-slate-900" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-700">Industry Category *</label>
                    <select value={formCategory} onChange={(e) => { setFormCategory(e.target.value); const g = CATEGORY_GROUPS.find((cg) => cg.name === e.target.value); if (g) setFormSubCategory(g.subcategories[0]); }} className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-orange-500 outline-none bg-white font-medium text-slate-900">
                      {CATEGORY_GROUPS.map((cat) => <option key={cat.name} value={cat.name}>{cat.name}</option>)}
                    </select>
                  </div>
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-700">Subcategory *</label>
                    <select value={formSubCategory} onChange={(e) => setFormSubCategory(e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-orange-500 outline-none bg-white font-medium text-slate-900">
              ...
              <div className="space-y-4 pt-6 border-t border-slate-100">
                <div className="flex items-center gap-2 text-slate-900 font-extrabold text-base">
                  <MapPin className="w-5 h-5 text-orange-600" /> Ikorodu Address & Contact Details
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1 sm:col-span-2">
                    <label className="text-xs font-bold text-slate-700">Physical Business Address in Ikorodu *</label>
                    <input type="text" required placeholder="e.g. Shop 4, Ita Elewa Market Road, Beside First Bank, Ikorodu" value={formAddress} onChange={(e) => setFormAddress(e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-orange-500 outline-none text-slate-900" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-700">Ikorodu Zone *</label>
                    <select value={formZone} onChange={(e) => { setFormZone(e.target.value); const z = IKORODU_ZONES.find((z) => z.name === e.target.value); if (z) setFormArea(z.areas[0]); }} className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-orange-500 outline-none bg-white font-medium text-slate-900">
                      {IKORODU_ZONES.map((zone) => <option key={zone.name} value={zone.name}>{zone.name}</option>)}
                    </select>
                  </div>
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-700">Neighbourhood / Area *</label>
                    <select value={formArea} onChange={(e) => setFormArea(e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-orange-500 outline-none bg-white font-medium text-slate-900">
                      {(IKORODU_ZONES.find((z) => z.name === formZone)?.areas || ALL_IKORODU_AREAS).map((a) => <option key={a} value={a}>{a}</option>)}
                    </select>
                  </div>
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-700">WhatsApp Phone Number *</label>
                    <input type="tel" required placeholder="e.g. 08031234567" value={formWhatsapp} onChange={(e) => setFormWhatsapp(e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-orange-500 outline-none font-semibold text-emerald-700" />
                  ...
                  <div className="space-y-1">
                    ...
                    <input type="tel" placeholder="e.g. 08029876543" value={formPhone} onChange={(e) => setFormPhone(e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-orange-500 outline-none text-slate-900" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-700">Operating Hours</label>
                    <input type="text" placeholder="e.g. Mon - Sat: 8:00 AM - 7:00 PM" value={formOpeningHours} onChange={(e) => setFormOpeningHours(e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-orange-500 outline-none text-slate-900" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-700">Instagram Handle (Optional)</label>
                    <input type="text" placeholder="e.g. @ikorodu_fashion_hub" value={formInstagram} onChange={(e) => setFormInstagram(e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-orange-500 outline-none text-slate-900" />
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3 pt-6 border-t border-slate-100">
                <div className="flex items-center gap-2 text-slate-900 font-extrabold text-base">
                  <FileText className="w-5 h-5 text-orange-600" /> Business Description & Services
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-700">Store Description *</label>
                  <textarea rows={4} required placeholder="Describe your business services, products offered, delivery capabilities in Ikorodu..." value={formDescription} onChange={(e) => setFormDescription(e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-orange-500 outline-none leading-relaxed text-slate-900" />
                </div>
              </div>
              ...
              <div className="pt-6 border-t border-slate-100 flex items-center justify-end gap-3">
                ...
                <button type="submit" className="bg-orange-600 hover:bg-orange-700 text-white font-black px-8 py-3.5 rounded-xl shadow-md transition text-sm flex items-center gap-2">
          ...
          <div className="space-y-8">
            <div className="bg-gradient-to-r from-slate-900 via-emerald-950 to-slate-900 rounded-3xl p-6 sm:p-8 text-white shadow-xl relative overflow-hidden">
              <div className="relative z-10 max-w-2xl space-y-3">
                ...
                <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">Boost your business visibility across Ikorodu. Promotions are activated upon manual payment verification by the team.</p>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-6 mt-6 border-t border-white/10 text-xs relative z-10">
                {[
                  { label: 'Active Campaigns', value: myActivePromotions.length, color: 'text-emerald-400' },
                  { label: 'Pending Verification', value: myPendingPromotions.length, color: 'text-amber-400' },
                  { label: 'Expired Campaigns', value: myExpiredPromotions.length, color: 'text-slate-400' },
                  { label: 'Rejected Requests', value: myRejectedPromotions.length, color: 'text-rose-400' },
                ].map(({ label, value, color }) => (
                  <div key={label} className="bg-white/5 p-3.5 rounded-2xl border border-white/10">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-xs">
                {PROMOTION_PACKAGES.map((pkg) => (
                  <div key={pkg.type} className="bg-white rounded-3xl p-5 border border-slate-200 shadow-xs hover:shadow-md hover:border-emerald-400 transition flex flex-col justify-between space-y-4">
                    ...
                    <button onClick={() => setSelectedPromoPackage(pkg)} className="w-full bg-slate-900 hover:bg-emerald-600 text-white font-extrabold py-3 min-h-[44px] rounded-2xl text-xs transition flex items-center justify-center gap-1.5 cursor-pointer shadow-xs">
            ...
            <div className="bg-white rounded-3xl p-6 border border-slate-200 space-y-5 shadow-xs">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-2xl text-xs font-bold overflow-x-auto">
                  {(['all', 'pending_verification', 'active', 'expired', 'rejected'] as const).map((filter) => (
                    <button key={filter} onClick={() => setPromoTabFilter(filter)} className={`px-3 py-1.5 rounded-xl transition whitespace-nowrap ${promoTabFilter === filter ? 'bg-white text-slate-900 shadow-xs font-extrabold' : 'text-slate-600 hover:text-slate-900'}`}>
              ...
              {myPromotions.length === 0 ? (
                <div className="text-center py-12 bg-slate-50 rounded-2xl border border-dashed border-slate-200 space-y-3">
                  ...
                  <p className="text-xs text-slate-500 max-w-sm mx-auto">Select a package above to feature your store or products on the homepage.</p>
                </div>
              ) : (
                <div className="space-y-3">
                  {myPromotions
                        ...
                        <div key={promo.id} className="p-4 rounded-2xl bg-slate-50 border border-slate-200 hover:border-slate-300 transition flex flex-col md:flex-row md:items-center justify-between gap-4">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {(!isPending || promo.status === 'expired' || promo.status === 'rejected') && (
                              <button
                                onClick={() => setSelectedPromoPackage(matchingPkg || PROMOTION_PACKAGES[0])}
